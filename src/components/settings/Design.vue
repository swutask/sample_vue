<template>
  <div v-if="settings" class="h-full w-full flex flex-col justify-between overflow-y-auto text-app-color">
    <div class="w-full max-w-[320px] mx-auto md:max-w-full md:pl-[100px] md:pr-50 pt-31 pb-50">
      <div v-if="showBackButton" class="flex items-center md:hidden" @click="back">
        <i class="text-12 mr-10 icon-keyboard-arrow-left opacity-50" />
        <p class="text-opacity-80">Back</p>
      </div>

      <h2 class=" mb-30 font-bold text-35 md:text-21">Design</h2>
      ...
      <div id="font-family" class="py-30 w-full">
        ...
        <div class="flex flex-wrap -mx-10 md:pr-50">
          <ButtonModule
      ...
      <div id="font-size" class="pt-30 pb-30">
        ...
        <div class="flex flex-wrap -mx-10">
          <ButtonModule
            ...
            :text="size"
            border-width=""
            class="mt-20 mx-5 border-2 rounded-8 h-40 py-5 px-15 hover:border-main-color hover:text-main-color transition-all duration-50"
...
const { settings, showBackButton, updateSettings, updateDefaultIcon } = useSettings()
