...
function textRange (node, from, to) {
  ...
  const f = from > 0 ? from : 0
  range.setStart(node, f)
  return range
}
...
class Menu {
  constructor ({ options, editorView }) {
    ...
    this.editorView.dom.addEventListener('dragstart', this.dragstartHandler)
    this.editorView.dom.addEventListener('drop', this.dragstartHandler)
  ...
  update (view, lastState) {
    ...
    this.left = Math.round(Math.min(box.width - (el.width / 2), Math.max(left, el.width / 2)))
  ...
  destroy () {
    ...
    this.editorView.dom.removeEventListener('drop', this.dragstartHandler)
    this.editorView.dom.removeEventListener('dragstart', this.dragstartHandler)
